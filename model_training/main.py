...
def get_data_for_datatype(id):
    ...
    filename = f"data/dataset_type_{id}.json"
    with open(filename) as data_file:
        file = json.load(data_file)
        data = []
        for value in file.values():
            new_data = {}
            new_data['question'] = value['question']
            for i in range(1, 5):
                new_data[str(i)] = value[str(i)]
...
if __name__ == "__main__":
    ...
    torch.save(appr, 'model_pytorch.pt')